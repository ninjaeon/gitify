  ...
  mb.on('ready', () => {
    ...
    ipc.on(
      ...
      (_event, showWindowOnStartupBoolean) => {
        ...
        // const preActionIsVisible = mb.window ? mb.window.isVisible() : 'N/A'; // Retain for context if needed, but not logged

        if (showWindowOnStartupBoolean) {
          if (!mb.window?.isVisible()) { // Re-check
            mb.showWindow();
          }
        } else {
...
const handleURL = (url: string) => {
  if (url.startsWith(`${protocol}://`)) {
    ...
    // Retain a general log that OAuth is showing the window, but remove visibility specifics
    logInfo('main:handleURL', 'OAuth redirect: Ensuring window is visible.');
    mb.showWindow();
  }
};
